...
### 2026年3月5日 发布1.9.7版本 (R78)

更新日志(1.9.6->1.9.7)

1. 修复某些场景下列车径路相关铺画警告不合理的问题。
2. 修复在“列车径路管理”面板中删除所有列车径路导致程序闪退的问题。
3. 优化“标尺排图”功能中各表格默认列宽等细节。

### 2026年2月24日 发布1.9.6版本 (R77)

更新日志 (1.9.5->1.9.6)

1. 对于根据列车径路铺画运行线的列车，在可能存在因缺失车站时刻信息而造成缺失运行线（片段）时，给出警告信息。
2. 新增“快速推定（全局）”功能，原版中的快速推定功能现在改为“快速推定（当前线路）”。此功能对列车的所有运行线进行时刻插值（不考虑起停附加时分）。如果列车是依据列车径路铺画的，则此功能推定时会补充列车径路起点至终点间的所有车站信息，即使涉及到对既有运行线的外推。这种外推性质的推定依据里程而不是标尺，无论运行图以里程还是标尺安排纵坐标。
3. 新增创建反向列车径路功能。若列车径路的所有节点站都是双向通过站，则可以由此功能创建反向的列车径路，原径路不受影响。
4. 允许设置最大跨越站数为-1，此时禁用自动运行线管理，只采用基于列车径路的运行线铺画。
5. 增加指定列车径路、清空列车径路的批量操作。
6. 在运行图资源管理器中新增一些操作入口。

### 2026年2月17日 发布1.9.5版本 (R76)

更新日志 (1.9.4->1.9.5)

1. 新增列车信息汇总功能（工具栏页面 `列车(3)` | 信息汇总），包含各列车数据的简明表格，支持导出CSV。注意如果运行图所含列车数量较大，此功能的计算可能需要一定的时间。
2. 新增列车批量操作：删除时刻表中的连续重出站。此功能可删除列车时刻表中**连续**出现的相同车站。
3. 在线路数据库中，新增移动线路、分类到指定的分类下的功能，位于右键菜单。
4. 以CSV格式导入列车时刻表时，支持读取始发站、终到站。始发、终到站信息可附在每一行中。若同一列车提供了多处始发、终到站信息，则程序可能任选一处数据更新。同时在导出时增加始发终到站信息。
5. 部分细节。

### 2026年2月13日 发布1.9.4版本 (R75)

更新日志 (1.9.3->1.9.4)

1. 在列车副本、批量复制列车的操作中，为新添加的列车自动添加相同的列车径路配置（如果有）。
2. 在“区间车次表”的信息表中新增区间里程及对应的旅速、技速信息（如果可计算）。注意这些数据要求计算的区间内有连续铺画的列车运行线。
3. 修复特定场景下时刻编辑器异常、两段式车次名排序错误等问题。
4. 更新SARibbon库至2.5.7版本，ads库至4.4.1版本。此版本如需使用patch更新，请连同patch包中的所有dll文件一并复制到程序所在目录。

### 2025年9月9日 发布1.9.3版本 (R74)

更新日志 (1.9.2->1.9.3)

1. 在列车管理面板的批量操作中，如果未选择车次，则批量操作的对象为当前所有显示在列表中的车次，而不再报错。此操作可结合搜索以及列车筛选器使用。
2. 修复预置筛选器更新后，某些地方的筛选器下拉菜单列表没有更新的问题。
3. 在列车筛选器选择器中新增一个手动刷新筛选器的按钮。

### 2025年9月2日 发布1.9.2版本 (R73)

更新日志 (1.9.1->1.9.2)

1. 在列车管理面板中，支持列车筛选器。
2. 优化时刻编辑器控件：允许将时/分/秒中的任何一个部分在编辑过程中删成空白（这相当于令这部分为0）；在小时部分无法输入新的数字时自动跳转到分钟位。
3. 在列车管理面板表格中，可选的支持（为中国铁路车次格式优化的）特殊车次排序规则：按前缀-数字两部分分别排序，其中前缀按字符串比较，数字按数值排序。此处的前缀是指全车次开始部分最多三位的大小写字母或数字0；数字是指紧随前缀部分的任意数量的数字字符。选项开关位于`全局配置选项`中，默认关（简单的按车次名字符串排序）。请注意，如果车次名不是这种规则，排序结果可能不符合预期。
4. 支持设置交路连线的最大连线跨度（单位为分钟），设置项位于运行图显示设置中。如果交路连线的跨度超过设置值，运行图中的交路连线将不显示。默认0，表示不限制。注意，此设置项只影响绘图，不影响股道占用分析。
5. 修复一些已知问题。
...
1. 新增天窗数据的CSV格式输入/输出。操作位于运行图资源管理器相应项目的右键菜单，以及天窗编辑的面板中。
