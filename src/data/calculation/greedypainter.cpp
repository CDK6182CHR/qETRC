...
bool GreedyPainter::paint(const TrainName &trainName)
{
    ...
    bool flag = calForward(railint, _anchorTime, _anchor==_start && _localStarting);
    if (flag) {
        if (_localStarting) {
...
void GreedyPainter::addLog(std::unique_ptr<CalculationLogAbstract> log)
{
    ...
    if (log->toString() == "[杨家湾->观音山区间运行冲突 左冲突] 将[杨家湾]站[出发]时刻设置为[07:41:00] (对象: K290/1)") {
        qDebug() << "杨家湾！";
    }
...
bool GreedyPainter::calForward(std::shared_ptr<RailInterval> railint, const QTime& _tm, bool stop)
{
    ...
    // 注意以后的出发时间以这里面的为准！
    RailStationEventBase ev_start(qeutil::formerEventType(stop), _tm, RailStationEventBase::Pre, _dir);
    ...
    while (true) {
        if (tot_delay >= 24 * 3600) {
            // 没有可排的线位
            addLog(std::make_unique<CalculationLogBasic>(
                CalculationLogBasic::Terminated, st_from, ev_start.time, CalculationLogAbstract::Depart
                ));
            return false;
        }
        ...
        if (ev_conf) {
            ...
            if (!stop && st_from != _anchor) {
                ...
                if (qeutil::timeCompare(ev_start.time, ev_conf->time)) {
                    ...
                    tot_delay += gap_min;
                    ev_start.time = ev_conf->time.addSecs(gap_min);
        ...
        RailStationEventBase ev_stop(TrainEventType::SettledPass, tm_to, RailStationEventBase::Pre, _dir);
