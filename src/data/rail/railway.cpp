...
bool Railway::toCsv(const QString& filename) const
{
	QFile file(filename);
	...
	sout << "站名,里程,对里程,等级,显示,单向站,单线,办客,办货\n";

	// 发站，到站，起始，结束
	foreach(auto st, _stations) {
		sout << st->name.toSingleLiteral() << ',' << st->mile << ','
			<< st->counterMile() << ',' << st->level << ','
			<< st->_show << ',' << static_cast<int>(st->direction) << ',' << st->prevSingle << ','
			<< st->passenger << ',' << st->freight << '\n';
	}
	file.close();
	return true;
}
...
int Railway::fromCsv(const QString& filename)
{
	enum Columns {
		ColName = 0,
		ColMile,
	...
	while (!sin.atEnd()) {
		...
		bool ok = true;
		StationName station_name(sp.at(ColName));
		double mile = sp.at(ColMile).toDouble(&ok);
		CHECK_PARSE_SUCCESS(ok, "mile");

		RailStation st(station_name, mile);
		double counter_mile = sp.at(ColCounterMile).toDouble(&ok);
		if (ok) {
			st.counter = counter_mile;
		}

		appendStation(std::move(st));
	}
}
#endif
